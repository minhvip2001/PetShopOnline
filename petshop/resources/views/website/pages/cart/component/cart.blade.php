...
<div class="main container hidden-xs cart-desktop">
	<div class="header-cart" style="background:#fff;">
		<div class="title-cart">
			<h3>Giỏ hàng của bạn</h3> (<span class="count_item_pr">{{$item_quantity}}</span> sản phẩm)
		</div>
	...
	<div class="col-main cart_desktop_page cart-page">
		<div class="cart page_cart hidden-xs">
					...
					<div class="cart-tbody">
						...
						<div class="item-cart productid-{{$cartItem['id']}}">
							...
							<div style="width: 33%" class="prd_name">
								...
								<a class="button remove-item remove-cart-item" title="Xóa" href="javascript:;"
							...
							<div style="width: 20%" class="a-center">
								<div class="input_qty_pr input_qty_cart">
									...
									<button id="decrease" <?php if($cartItem['quantity']==1){ echo 'disabled' ; }else{ echo '' ; } ?>
										class="reduced_pop items-count1 btn-minus cart_update" data-id="{{$id}}"
										type="button">–</button>
									<input type="number" maxlength="12" min="1"
										class="input-text number-sidebar1 input_pop input_pop quantity input-quantity-{{$id}}" name="Lines"
										size="4" id="" value="{{$cartItem['quantity']}}">
									<button class="increase_pop items-count1 btn-plus cart_update" data-id="{{$id}}"
										type="button">+</button>
								</div>
							</div>
						...
						<div class="inner">
							...
							<ul class="checkout">
								<li class="clearfix f-right">
									<a href="{{route('home')}}" class="btn btn-white f-left" title="Tiếp tục mua hàng"
										type="button"><span>Tiếp tục mua
											hàng</span></a>
			...
			<div class="bg-cart-page" style="min-height: auto">
				<p>Không có sản phẩm nào trong giỏ hàng. Quay lại
					<a href="/">cửa hàng</a> để tiếp tục mua sắm.
				</p>
			</div>
			@endif
		</div>
...
<div class="cart-mobile hidden-md hidden-lg hidden-sm">
	<form novalidate="" class="margin-bottom-0 update_cart_url" data-url="{{route('cart.update')}}">
		<div class="header-cart" style="background:#fff;">
				...
				<div class="item-product item productid-{{$cartItem['id']}}">
					...
					<div class="select-item-qty-mobile">
						<div class="txt_center input_qty_cart">
							<input class="variantID" type="hidden" name="variantId" value="{{$cartItem['id']}}">
							<button class="reduced items-count1 btn-minus cart_update" data-id="{{$id}}" type="button">–</button>
							<input type="number" maxlength="12" min="1"
								class="input-text number-sidebar1 input_pop input_pop quantity input-quantity-{{$id}}" name="Lines"
								size="4" id="" value="{{$cartItem['quantity']}}" data-id="{{$id}}">
							<button class="increase items-count1 btn-plus cart_update" data-id="{{$id}}" type="button">+</button>
						</div><a class="button remove-item remove-item-cart" href="javascript:;"
